...
def convolution(valid_dataset, test_dataset):
	with graph.as_default():
		...
		def model(data):
			...
			reshape = tf.reshape(max_pool, [shape[0], shape[1]*shape[2]*shape[3]]) #shape[0] ~~batch_size, shape[1]~~out_height, shape[2]~~out_width, shape3~~depth
			#print("reshape.dim", reshape.shape);
