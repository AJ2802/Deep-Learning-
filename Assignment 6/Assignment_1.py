...
def graphForLSTM(num_nodes, vocabulary_size, batch_size, num_unrollings):
	...
	with graph.as_default():
		...
		###############################################################################################
		#############################Assignment Addition code Begin####################################
		###############################################################################################
		# Concatenate of 3 kinds of parameters (ix, fx, cx, ox), (im, fm, cm, om) and (ib, fb, cb, ob)
		sx = tf.concat([ix, fx, cx, ox], 1) #the matrix looks like [ix fx cx ox]
		sm = tf.concat([im, fm, cm, om], 1) #the matrix looks like [im fm cm om]
		sb = tf.concat([ib, fb, cb, ob], 1) #the matrix looks like [ib fb cb ob]
		
		###############################################################################################
		##############################Assignment Addition code End#####################################
		###############################################################################################
		...
		def lstm_cell(i ,o , state): #i is a batch which is batch_size*vocabulary_size dimension o is a matrix of batch_size*num_nodes, state is a matrix of batch_size*num_nodes
			...
			overall = tf.matmul(i,sx) + tf.matmul(o,sm) + sb
